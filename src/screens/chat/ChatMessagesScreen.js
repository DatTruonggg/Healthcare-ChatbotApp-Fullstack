import {
  ...
  import Icon from "react-native-vector-icons/Feather"; // Import Feather from react-native-vector-icons
  ...
  const ChatMessagesScreen = () => {
	...
	const fetchMessages = async () => {
	  try {
		const response = await fetch(
		  `http://192.168.199.149:8080/messages/${userId}/${recepientId}`
		);
	...
	useEffect(() => {
	  const fetchRecepientData = async () => {
		try {
		  const response = await fetch(
			`http://192.168.199.149:8080/user/${recepientId}`
		  );
	...
	const handleSend = async (messageType, imageUri) => {
	  try {
		...
		const response = await fetch("http://192.168.199.149:8080/messages", {
		  method: "POST",
		  body: formData,
		});
	...
	const deleteMessages = async (messageIds) => {
	  try {
		const response = await fetch("http://192.168.199.149:3002/deleteMessages", {
		  method: "POST",
		  headers: {
